...
4. **Close with self-authored cards** — only for the concepts I have understood, write Anki cards myself (with Claude as editor). Self-authored cards stick ~3× better than canned decks.
...
Scripts use [`uv`](https://docs.astral.sh/uv/) and bootstrap their own Python dependencies on first run — install `uv` once (`curl -LsSf https://astral.sh/uv/install.sh | sh`), make sure `ffmpeg` is on your PATH, and the rest is automatic.

## Spaced repetition

Anki is the SR system. Decks live in `cards/<topic>.txt` as Anki-importable plain-text files. To import:

1. Open Anki → *File → Import*.
2. Pick the `.txt` file.
3. Confirm the column mapping; click *Import*.

The card-design philosophy and exact format spec live in `.claude/skills/anki-card-builder/SKILL.md`. The skill is auto-discovered by Claude Code in this project.
