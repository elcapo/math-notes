...
`[x]` covered — all four blocks from material 01 are now in the spaced repetition rotation: functions (9 cards), function-families (13 cards), algebra (7 cards), trigonometry (8 cards). Total: 38 cards.
...
- **2026-05-09** — Challenge session on algebra block (linear functions from two points, rational functions from asymptotes + point, binomial expansion, difference of squares, deriving $\tan^2x+1=\sec^2x$ and $1+\cot^2x=\csc^2x$) and trigonometry block (unit circle as defining object, degrees↔radians conversion, complementary/negative/supplementary angles, sum/difference and double-angle identities, solving $\cos x=1/2$). Closed by authoring [15 cards in English across two subdecks](../../cards/calculus-foundations-algebra-trig.txt) (Algebra: 7 cards, Trigonometry: 8 cards).
